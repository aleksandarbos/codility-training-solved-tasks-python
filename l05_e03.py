...
# def solution(S, P, Q):
#     nucleoids = {'A': 1, 'C': 2, 'G': 3, 'T': 4}
#     S_vals = [nucleoids[val] for val in S] # O(n)
#     results = []

#     for i in range(len(P)): # len(P) = len(Q) = M => O(m)
#         start, stop = P[i], Q[i]
#         if S_vals[start] == 1 or S_vals[stop] == 1:
#             results.append(1)
#         else:
#             results.append(min(S_vals[start: stop+1])) # O(n)

#     return results


# time complexity: O(n) + O(m) => O(n+m)
# space complexity: m * O(n) => O(n*m)
def solution(S, P, Q):
    N = len(S)
